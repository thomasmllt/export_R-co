...
router.get("/", async (req, res) => {
  try {
    const result = await pool.query("SELECT * FROM mesures");
...
router.get("/:id", async (req, res) => {
  const { id } = req.params;
  try {
    const result = await pool.query("SELECT * FROM mesures WHERE id=$1", [id]);
...
router.post("/", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "INSERT INTO mesures (valeur, date, balise_id) VALUES ($1, $2, $3) RETURNING *",
